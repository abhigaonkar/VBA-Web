...
Dim cMaxBowDeflection As Variant
Dim cBowLength As Variant
...
On Error GoTo ErrHandler
    Select Case Description
        ...
        Case "HV OD Tolerance +"
            cOutsideDiameterPlusTolerance = CDec(specification)
        Case "HV OD Tolerance -"
            cOutsideDiameterMinusTolerance = CDec(specification)
        Case "HV Inside Diameter"
            cInsideDiameter = CDec(specification)
        Case "HV ID Tolerance +"
            cInsideDiameterPlusTolerance = CDec(specification)
        Case "HV ID Tolerance -"
            cInsideDiameterMinusTolerance = CDec(specification)
        Case "HV Wall"
            cWall = CDec(specification)
        Case "HV Wall Tolerance +"
            cWallPlusTolerance = CDec(specification)
        Case "HV Wall Tolerance -"
            cWallMinusTolerance = CDec(specification)
        Case "HV Length"
         '   cLength = CDec(specification)
        Case "HV Length Tolerance +"
            cLengthPlusTolerance = CDec(specification)
        Case "HV Length Tolerance -"
            cLengthMinusTolerance = CDec(specification)
        Case "HV Maximum Bow Deflection"
            cMaxBowDeflection = CDec(specification)
        Case "HV Bow Length"
            cBowLength = CDec(specification)
        Case "HV Bow Limit"
            cBowLimit = CDec(specification)
    End Select
Exit Sub

ErrHandler:
        StdErrorHandler "Error " & Err & ": " & Error(Err) & " in loadClassValues of clsMaterial"
...
Function getSpecsAsString() As String
   ...
   ' getSpecs
    ...
    result = result & vbCrLf
    result = result & "Length+" & vbTab & vbTab & cLengthPlusTolerance
    result = result & vbCrLf
    result = result & "Length-" & vbTab & vbTab & cLengthMinusTolerance
    result = result & vbCrLf
    result = result & "Max Bow Dflctn" & vbTab & cMaxBowDeflection
    result = result & vbCrLf
    result = result & "Bow length" & vbTab & vbTab & cBowLength
...
Sub calculateMaxAndMins()
    ...
    If cOutsideDiameterPlusTolerance <> 0 And cOutsideDiameterMinusTolerance <> 0 Then ' Check if tolerances are defined
        ...
        Sheet2.Cells(1, 24).Value = cOutsideDiameterMax
        Sheet2.Cells(2, 24).Value = cOutsideDiameterMin
    End If
    ...
    If cWallPlusTolerance <> 0 And cWallMinusTolerance <> 0 Then ' Check if tolerances are defined
        ...
        Sheet2.Cells(3, 24).Value = cWallMax
        Sheet2.Cells(4, 24).Value = cWallMin
    End If
    ' Good place to save length
    Sheet2.Cells(5, 24).Value = Round(cLength / 25.4, 2)
End Sub
