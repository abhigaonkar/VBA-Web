...
Dim cPrintArray As Variant
Dim cSerialized As Boolean
...
'        Exit Function
    ...
    If keepGoing Then
 '       cor6GUI.FindById("wnd[0]/usr/tabsTABSTRIP_0150/tabpMGLE/ssubVAR_CNF_10:SAPLCORU:5850/radCORUF-ENDRU").Select    'set the final confirmation status
...
'        cor6GUI.FindById("wnd[0]/usr/tabsTABSTRIP_0150/tabpRZUS/ssubVAR_CNF_10:SAPLCORU:5870/ctxtAFRUD-PERNR").Text = "97747"
  ...
  '              MsgBox "Say something about the material application failure and continuing step 40."
        ...
        If SAPStatus.IsThereAModalWindow Then
            SAPStatus.CheckModalWindow
            cor6GUI.FindById("wnd[1]/usr/btnCANCEL").Press
        End If
    End If
...
On Error GoTo ErrHandler
    ...
    With cor6GUI
        ...
        'to take care of custom batch
        If cTubeSerial <> "" Then
            .FindById("wnd[0]/usr/subTABLE:SAPLCOWB:0500/tblSAPLCOWBTCTRL_0500/ctxtCOWB_COMP-CHARG[5,0]").Text = cTubeSerial
            .FindById("wnd[0]").SendVKey 0
            If SAPStatus.CheckModalWindow Then
                .FindById("wnd[1]").SendVKey 4
                .FindById("wnd[2]/tbar[0]/btn[0]").Press
                .FindById("wnd[1]/tbar[0]/btn[0]").Press
            End If
            .FindById("wnd[0]").SendVKey 0
            If Not cSerialized Then .FindById("wnd[0]/usr/subTABLE:SAPLCOWB:0500/tblSAPLCOWBTCTRL_0500/ctxtCOWB_COMP-CHARG[5,1]").Text = cTubeSerial
            .FindById("wnd[0]").SendVKey 0
        End If
        If Not cSerialized Then
            Dim thisLoc As String
            thisLoc = .FindById("wnd[0]/usr/subTABLE:SAPLCOWB:0500/tblSAPLCOWBTCTRL_0500/ctxtCOWB_COMP-LGORT[4,0]").Text
            If MsgBox("This material will be stored in " & thisLoc & ". Is that OK?", vbYesNo) = vbNo Then
                thisLoc = InputBox("Where would you like it stored in SAP?")
                If Len(thisLoc) < 4 Then thisLoc = addZeros(CInt(thisLoc))
                .FindById("wnd[0]/usr/subTABLE:SAPLCOWB:0500/tblSAPLCOWBTCTRL_0500/ctxtCOWB_COMP-LGORT[4,0]").Text = thisLoc
            End If
            SAPRowNum = 1
        Else
            SAPRowNum = 0
            .FindById("wnd[0]/usr/subTABLE:SAPLCOWB:0500/tblSAPLCOWBTCTRL_0500").GetAbsoluteRow(0).Selected = True
        End If
        ...
        Do While x < cApplyNumMatl
            If cMatlArray(x, 0) <> "" Then 'make sure there's something in this row
        ...
        If Not cSerialized Then
            If SAPStatus.SAPWindowName = "Print Label" Then
                If printYes Then ' Check if printing is enabled (using a variable printMe)
                    ...
                    For r = 0 To 2
                        If cPrintArray(r, 0) <> "" Then
                            cor6GUI.FindById("wnd[0]/usr/txtP_IGMNG").Text = cPrintArray(r, 1)
                            cor6GUI.FindById("wnd[0]/usr/txtP_NUM").Text = cPrintArray(r, 0)
            
                            cor6GUI.FindById("wnd[0]/tbar[1]/btn[8]").Press ' Press "Enter" (likely F8)
                            
                            If SAPStatus.IsThereAModalWindow Then cor6GUI.FindById("wnd[1]/tbar[0]/btn[0]").Press ' Press "Continue" in the modal window
                        End If
                    Next r
...
'            cor6GUI.FindById("wnd[1]/usr/btnCANCEL").Press
    ...
    If SAPStatus.IsThereAModalWindow Then cor6GUI.FindById("wnd[1]/tbar[0]/btn[0]").Press
...
Property Let printArray(ByVal printArray As Variant)  'array for label printing
On Error GoTo ErrHandler

    cPrintArray = printArray

    Exit Property
ErrHandler:
    
    StdErrorHandler "Error " & Err & ": " & Error(Err) & " in Let printArray of clsCOR6."
...
End Property
Property Let serialized(ByVal newData As Boolean)
On Error GoTo ErrHandler
...
ErrHandler:
    ...
    StdErrorHandler "Error " & Err & ": " & Error(Err) & " in Let serialized of clsCOR6."
