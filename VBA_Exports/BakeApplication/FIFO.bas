...
Function findMaterial(itemNum As String, batch As String, Optional sloc As String) As Variant
    ...
    'don't include 0400 in results
    thisGUI.FindById("wnd[0]/usr/ctxtLGORT-LOW").Text = "0400"
    thisGUI.FindById("wnd[0]/usr/ctxtLGORT-LOW").SetFocus
    thisGUI.FindById("wnd[0]/usr/ctxtLGORT-LOW").CaretPosition = 2
    thisGUI.FindById("wnd[0]").SendVKey 2
    thisGUI.FindById("wnd[1]/usr/cntlOPTION_CONTAINER/shellcont/shell").SetCurrentCell 5, "TEXT"
    thisGUI.FindById("wnd[1]/usr/cntlOPTION_CONTAINER/shellcont/shell").SelectedRows = "5"
    thisGUI.FindById("wnd[1]/usr/cntlOPTION_CONTAINER/shellcont/shell").DoubleClickCurrentCell
    ...
    ElseIf b = 0 Then
        ...
        sandAtLoc(0, 3) = ""
    Else
        sandAtLoc(0, 2) = CDec(sandAtLoc(0, 2)) + CDec(sandAtLoc(1, 2))
    End If
...
ErrHandler:
    ' --- Error handling ---
    StdErrorHandler "Error " & Err & ": " & Error(Err) & " in findMaterial of FIFO Module" ' Call the standard error handler
End Function
